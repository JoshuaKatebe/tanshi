...
const Portfolio = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-800 text-white overflow-hidden">
      ...
      {/* Enhanced Navigation */}
      <motion.nav 
        className="fixed top-0 w-full z-50 bg-slate-900/90 backdrop-blur-md border-b border-blue-500/30 shadow-xl"
        initial={{ y: -100 }}
        animate={{ y: 0 }}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            ...
            <div className="hidden md:flex space-x-8">
              {['Home', 'About', 'Skills', 'Projects', 'Websites', 'Contact'].map((item) => (
                <motion.a
                  key={item}
            ...
            <motion.button
              className="md:hidden p-2 rounded-lg bg-blue-500/20 hover:bg-blue-500/30 transition-colors"
              whileTap={{ scale: 0.95 }}
        ...
        {isMenuOpen && (
          ...
          >
            <div className="flex flex-col items-center justify-center h-full space-y-8">
              {['Home', 'About', 'Skills', 'Projects', 'Websites', 'Contact'].map((item, index) => (
                <motion.a
                  key={item}
                  href={`#${item.toLowerCase()}`}
